...
// Mapa de nombre de ícono (string del backend) → componente Lucide
const ICONO_MAP = {
  Utensils,
  Car,
  Home: HomeIcon,
...
// Normaliza un string para comparación sin acentos ni mayúsculas
const catSlug = (str = '') =>
...
export default function Gastos() {
  ...
  const [listaGastosActiva, setListaGastosActiva] = useState([])
  ...
  useEffect(() => {
    const fetchGastos = async () => {
      try {
        ...
        setListaGastosActiva(res.data.data)
      } catch (err) {
        setError('No se pudieron cargar los gastos. Verifica tu conexión.')
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleEliminarGasto = async (id, nombre) => {
  ...
  const gastosFiltrados = listaGastosActiva.filter(gasto => {
    ...
    // catSlug normaliza acentos: 'Alimentación' → 'alimentacion', 'Entretenimiento' → 'entretenimiento'
    const coincideCategoria = catSeleccionada === 'todas' ||
      catSlug(gasto.categoria_nombre || '').startsWith(catSlug(catSeleccionada))
    return coincideTexto && coincideCategoria
  })

  const totalGeneralFiltrado = gastosFiltrados.reduce((acc, current) => acc + Number(current.monto), 0)

  const getIcono = (iconoNombre) => {
    const Comp = ICONO_MAP[iconoNombre] || Utensils
    return <Comp size={18} />
  }
  ...
  const agruparGastosPorFechaVisual = (lista) => {
    ...
    const grupos = {}
    lista.forEach(g => {
      // pg devuelve DATE como string 'YYYY-MM-DD'; substring(0,10) lo garantiza
      const fechaStr = (g.fecha || '').substring(0, 10)
      let labelFecha
      if (fechaStr === hoyStr) {
        labelFecha = 'HOY'
        ...
        const d = new Date(fechaStr + 'T12:00:00') // mediodía para evitar problemas de DST
        labelFecha = `${diasSemana[d.getDay()]}, ${parseInt(dia)} ${meses[parseInt(mes) - 1]}`
      }
      if (!grupos[labelFecha]) {
        grupos[labelFecha] = { titulo: labelFecha, totalDia: 0, items: [] }
      }
      grupos[labelFecha].items.push(g)
      grupos[labelFecha].totalDia += Number(g.monto)
    })
    ...
    <div className="h-full bg-[#0F1419] text-[#F5F7FB] pb-32 pt-4 px-4 overflow-y-auto select-none">
      ...
      {loading && (
        <div className="text-center py-12">
          <p className="text-xs text-neutral-500 font-medium">Cargando gastos...</p>
        </div>
      )}
            ...
            <div key={bloque.titulo} className="space-y-2">
              ...
              <div className="bg-[#1A1F2F]/40 border border-[#374151]/10 rounded-2xl overflow-hidden divide-y divide-[#374151]/20 shadow-sm">
                {bloque.items.map((gasto) => {
                  const catColor = gasto.categoria_color || '#8B949E'

                  return (
                    <div key={gasto.id} className="group p-3.5 flex items-center justify-between hover:bg-[#1A1F2F]/30 transition-colors">
                      <div className="flex items-center gap-3.5 min-w-0 flex-1">
                        <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0" style={{ backgroundColor: `${catColor}25`, color: catColor }}>
                          {getIcono(gasto.categoria_icono)}
                        </div>

                        <div className="min-w-0 flex-1">
                          <h4 className="text-xs font-bold text-[#F5F7FB] truncate">{gasto.nombre}</h4>
                          <div className="flex items-center gap-2 mt-1 flex-wrap">
                            <span className="text-[9px] font-bold px-2 py-0.5 rounded-full border" style={{ backgroundColor: `${catColor}10`, color: catColor, borderColor: `${catColor}30` }}>
